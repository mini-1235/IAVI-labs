...
object_points = np.zeros((Grid_Point_Col * Grid_Point_Row, 3), np.float32)
object_points[:, :2] = np.mgrid[0:Grid_Point_Col, 0:Grid_Point_Row].T.reshape(-1, 2)
...
while True:
    ...
    ret, corners = cv2.findChessboardCorners(gray, (Grid_Point_Col, Grid_Point_Row), None)

    if ret:
        object_points_list.append(object_points)
        corners = cv2.cornerSubPix(gray, corners, (11, 11), (-1, -1), criteria)
        image_points_list.append(corners)
        cv2.drawChessboardCorners(frame, (Grid_Point_Col, Grid_Point_Row), corners, ret)
